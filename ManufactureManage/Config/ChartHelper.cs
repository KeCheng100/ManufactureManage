...
namespace ManufactureManage.Config
{
    public class ChartHelper
    {
        ...
        public static void AddSeries(Chart chart, string seriesName, SeriesChartType chartType, Color color, Color markColor, bool showValue )
        {
            chart.Series.Add(seriesName);
            chart.Series[seriesName].ChartType = chartType;
            chart.Series[seriesName].Color = color;
            chart.Series[seriesName]["PieLabelStyle"] = "Outside";//将文字移到外侧 
            chart.Series[seriesName]["PieLineColor"] = "Black";//绘制黑色的连线。 
            chart.Series[seriesName].ToolTip = "#VAL";//鼠标停靠在饼状图上时，显示数值。
            if (showValue)
            {
        ...
        public static void SetTitle(Chart chart, string chartName, Color backColor, GradientStyle gradientStyle, ContentAlignment alignment, Font font, Docking docking, Color foreColor)
        {
            chart.Titles.Add(chartName);
            chart.Titles[0].BackColor = backColor;
            chart.Titles[0].BackGradientStyle = gradientStyle;
        ...
        /// <param name="xInterval">X轴的间距 0为默认</param>
        /// <param name="yInterval">Y轴的间距 0为默认</param>
